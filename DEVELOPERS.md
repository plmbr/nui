...
Definitions live in `internal/agents/api_builtins.go`. Availability is checked via `APIHarnessAvailable()` in `internal/agent/api_availability.go` (process env or `~/.nui/env.json` from Customize → Env vars). Model overrides use the same credential lookup (`resolveAPIModel` / `resolveAntigravityModel`). See [harness-design.md](dev/harness-design.md) §4 for ADL fields (`provider`, `model`, `baseURL`, `apiKeyEnv`).
